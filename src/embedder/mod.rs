...
impl Embedder {
	pub fn load(model_info: &ModelInfo, device: Device) -> Result<Self> {
		...
		let embedding_dim = config.get("hidden_size")
			.or_else(|| config.get("n_embd"))
			.or_else(|| config.get("dim"))
			.and_then(|v| v.as_u64())
			.ok_or_else(|| Error::ModelLoadFailed("Could not determine embedding dimension".to_string()))? as usize;
	...
	fn embed_tokens(&self, token_ids: &[u32]) -> Result<Tensor> {
		...
		let embedding_weight_name = tensor_list.iter()
			.find(|(name, _)| {
				name.contains("embeddings") && name.contains("word_embeddings") && name.ends_with("weight") ||
				name.ends_with("embed_tokens.weight") ||
				name.ends_with("wte.weight")
			})
			.map(|(name, _)| name.clone())
			.ok_or_else(|| Error::ModelLoadFailed("Could not find embedding weight tensor".to_string()))?;

		tracing::debug!("Using embedding tensor: {}", embedding_weight_name);

		let embeddings_weight = safetensors.load(&embedding_weight_name, &self.device)
			.map_err(|e| Error::EmbeddingError(format!("Failed to load embedding tensor: {}", e)))?;
