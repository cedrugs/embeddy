	...
	match device_str {
		...
		s if s.starts_with("cuda") => {
			...
			let ordinal = if parts.len() > 1 {
				parts[1].parse::<usize>()
					.map_err(|_| error::Error::InvalidInput(format!("Invalid CUDA device: {}", s)))?
			} else {
				0
			};
			Device::new_cuda(ordinal)
				.map_err(|e| error::Error::ConfigError(format!("Failed to initialize CUDA device: {}", e)))
		}
		_ => Err(error::Error::InvalidInput(format!("Unknown device: {}", device_str))),
	}
}
...
async fn main() -> Result<()> {
	...
	match cli.command {
		...
		Commands::Run { model, text, device } => {
			if text.is_empty() {
				return Err(error::Error::InvalidInput("No text provided. Use --text \"your text\"".to_string()));
			}
